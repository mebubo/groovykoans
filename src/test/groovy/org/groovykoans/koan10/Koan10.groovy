...
class Koan10 extends GroovyTestCase {
    ...
    void test01_XmlSlurpersReader() {
        ...
        def movieCount
        // ------------ START EDITING HERE ----------------------
        def xml = new XmlSlurper().parse(Koan10.getResourceAsStream('movies.xml'))
        ...
        // ------------ START EDITING HERE ----------------------
        moviesWithThe = xml.movie.title.collect {
            it.text()
        }.findAll {
            it.toLowerCase().contains('the')
        }
        // ------------ STOP EDITING HERE  ----------------------
    ...
    void test02_XmlSlurpersReader2() {
        ...
        // ------------ START EDITING HERE ----------------------
        def xml = new XmlSlurper().parse(Koan10.getResourceAsStream('movies.xml'))
        sortedList = xml.movie.list().sort { m1, m2 ->
            def yearCompare = m1.year.text() <=> m2.year.text()
            yearCompare != 0 ? yearCompare : m1.title.text() <=> m2.title.text()
        }.collect {
            it.title.text()
        }
        // ------------ STOP EDITING HERE  ----------------------
    ...
    void test06_AntBuilderChecksum() {
        ...
        builder.checksum(file: "$baseDir/movies.xml", property: 'checksum')
        actualChecksum = builder.project.properties.checksum
        // ------------ STOP EDITING HERE  ----------------------
