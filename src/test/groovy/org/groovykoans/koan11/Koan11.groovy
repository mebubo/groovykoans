...
class Koan11 extends GroovyTestCase {
    ...
    void test02_MoreOnInsertingData() {
        ...
        Sql.withInstance('jdbc:h2:mem:groovyDb2', 'sa', '', 'org.h2.Driver') { db ->
            ...
            // ------------ START EDITING HERE ----------------------
            def resource = Koan11.getResource('cast.txt')
            new File(resource.toURI()).eachLine {
                db.execute('insert into Person (FIRSTNAME, LASTNAME) values (?, ?)', it.split())
            }
            // ------------ STOP EDITING HERE  ----------------------
            ...
            // ------------ START EDITING HERE ----------------------
            def xml = new XmlSlurper().parse(Koan11.getResourceAsStream('cast2.xml'))
            xml.character.@actor*.text().each {
                db.execute('insert into Person (FIRSTNAME, LASTNAME) values (?, ?)', it.split())
            }
            ...
            // ------------ START EDITING HERE ----------------------
            person.add(firstname: 'Madame', lastname: 'Michu')
            // ------------ STOP EDITING HERE  ----------------------
